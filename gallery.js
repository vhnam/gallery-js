function Gallery(root, time, photos) {

    this.dom = document.getElementById(root);
    this.photos = photos;
    this.size = photos.length;
    this.time = time;

    this.createGalleryItems = function () {
        var ul, li, img, p, i;
    ...
    this.resetItems = function () {
        for (var i = 0; i < this.size; i++) {
            li = document.getElementById(`gallery-${i}`);
            if (li)
                li.classList.remove('gallery__item--active');
        }
    }

    this.getPositionForPhoto = function(element) {
    ...
    this.run = function () {
        var i = 0,
            _size = this.size,
            _time = this.time;
            funcResetItems = this.resetItems.bind(this);
            funcGetPositionForPhoto = this.getPositionForPhoto.bind(this);
        ...
        setInterval(function () {
            if (i === _size) {
                ...
                li  = document.getElementById(`gallery-${i}`);
                li.classList.add('gallery__item--active');
                funcGetPositionForPhoto(li);
            }
    ...
    this.setup = function() {
        var list = this.createGalleryItems();
